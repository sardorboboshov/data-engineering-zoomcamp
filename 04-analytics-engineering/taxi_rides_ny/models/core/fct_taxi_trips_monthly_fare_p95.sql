...
SELECT 
    ...
    PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY fare_amount) AS p90_sale_amount,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY fare_amount) AS p95_sale_amount,
    PERCENTILE_CONT(0.97) WITHIN GROUP (ORDER BY fare_amount) AS p97_sale_amount
